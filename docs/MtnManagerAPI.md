...
# **getAppReport**
```swift
    open class func getAppReport(acceptLanguage: String? = nil, completion: @escaping (_ data: AppReport?, _ error: Error?) -> Void)
```
...
// Get app report
MtnManagerAPI.getAppReport(acceptLanguage: acceptLanguage) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
